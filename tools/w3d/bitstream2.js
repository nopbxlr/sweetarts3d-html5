...
class BitStreamRead {
  ...
  // opts: variant flags for IFX v2 archaeology:
  //   ufFirst: skip underflow bits BEFORE the code bit in _fillCode
  //   flushUfAlways: in _updateStateAndAdvance, flush underflow into bitCount even when bitCount===0
  //   noEscCount: dynamic histogram does not count escape occurrences
  //   noLearn: escapes do not add the value to the histogram
  //   oneFastStatic: static reads use one fast step instead of two
  constructor(data, elephant, opts) {
    this.opts = opts || {};
  ...
  _updateStateAndAdvance(uLow, uHigh, twoFastSteps) {
    ...
    if (bitCount > 0 || this.opts.flushUfAlways) {
  ...
  readSymbolContextStatic(ctx) {
    ...
    this._updateStateAndAdvance(uLow, uHigh, !this.opts.oneFastStatic);
    return value;
  }
  ...
  readSymbolContextDynamic(ctx) {
    ...
    const uHigh = uLow0 - 1 + Math.floor(range * (valueCumFreq + valueFreq) / total);
    const uLow = uLow0 + Math.floor(range * valueCumFreq / total);
    if (!(value === 0 && this.opts.noEscCount)) h.addSymbol(value);
  ...
  readCompressedU32(ctx) {
    if (ctx !== 0 && ctx < ACMaxRange) {
      ...
      if (!(ctx > ACStaticFull) && !this.opts.noLearn) this.getContext(ctx).addSymbol(v + 1);
      return v;
    }
    return this.readU32();
  }
